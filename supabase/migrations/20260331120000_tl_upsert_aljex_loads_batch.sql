-- VPS /insert-aljex-loads: explicit upsert — on conflict refresh lane/rate fields only.
-- NOTE: Postgres has no UNIQUE(load_number) alone; arbiter matches loads_agency_id_template_type_load_number_key.
...
COMMENT ON FUNCTION public.tl_upsert_aljex_loads_batch(jsonb) IS
  'Called from VPS tl-trigger /insert-aljex-loads: insert or update (lane/rate fields only on conflict).';
